...
function renderRiwayatUnduhan() {
  ...
  summary.innerHTML = `📁 Modifikasi: ${mods.length} | 🎮 Level: ${levels.length}`;

  const lastItem = [...mods, ...levels].sort((a, b) => new Date(b.updatedAt || b.createdAt) - new Date(a.updatedAt || a.createdAt))[0];
  ...
  updateChart(mods.length, levels.length);
}
...
let chart;
function updateChart(modCount, levelCount) {
  const ctx = document.getElementById('riwayatChart').getContext('2d');
  if (chart) chart.destroy();
  chart = new Chart(ctx, {
    type: 'doughnut',
    data: {
      labels: ['Modifikasi', 'Level'],
      datasets: [{
        data: [modCount, levelCount],
